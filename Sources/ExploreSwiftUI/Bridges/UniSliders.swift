...
public struct UniSliderTickInfo: Identifiable, @unchecked Sendable {
    public let id = UUID()
...
/// A structure that represents a tick mark in an uni slider.
///
...
/// Example:
/// ```swift
/// UniSliderTick(0.5) { Text("Middle") }
/// ```
public struct UniSliderTick: View {
    public let value: Double
    public let label: AnyView?
...
public struct UniSliderTickContentForEach<
...
extension View {
    ...
    /// ```swift
    /// Slider(value: $val)
    ///     .uniSliderTint(.orange)
    /// ```
    @ViewBuilder
    public func uniSliderTint(_ color: Color?) -> some View {
        if let color = color {
    ...
    /// Configures tick marks for an uni slider (Future Support).
    @ViewBuilder
    public func uniSliderTicks<T: View>(@ViewBuilder _ ticks: @escaping () -> T) -> some View {
...
/// An uni slider that supports tick marks and dynamic value labels.
///
...
/// Example:
/// ```swift
/// UniTickedSlider(value: $volume, in: 0...100) {
///     Text("Volume")
/// } currentValueLabel: {
///     Text("\(Int(volume))%")
/// } minimumValueLabel: {
...
@available(tvOS, unavailable)
@available(*, deprecated, message: "Use UniTickedSlider init(..., tickLabel:) instead.")
public struct UniTickedSliderLabeled<
    V: BinaryFloatingPoint & Hashable,
    Label: View,
    CurrentValueLabel: View,
    ValueLabel: View,
    TickLabel: View
>: View where V.Stride: BinaryFloatingPoint {
    private let slider: UniTickedSlider<V, Label, CurrentValueLabel, ValueLabel>
