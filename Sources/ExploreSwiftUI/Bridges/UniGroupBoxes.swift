...
extension View {
    ...
    /// Example:
    /// ```swift
    /// UniGroupBox(label: Text("Settings")) {
    ///     Toggle("Notifications", isOn: $isOn)
    /// }
    /// .uniGroupBoxBackgroundStyle(.secondary.opacity(0.1))
    /// ```
    @ViewBuilder
    public func uniGroupBoxBackgroundStyle<S: ShapeStyle>(_ style: S) -> some View {
