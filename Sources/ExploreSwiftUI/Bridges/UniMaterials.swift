...
extension View {
    ...
    /// Applies an uni material as the background of the view with cross-platform support.
    ///
    ...
    /// - **iOS 13-14**: Falls back to a custom polyfill (like `UIVisualEffectView` via `UniMaterial`).
    ///
    ...
    /// ```swift
    /// MyView()
    ///     .uniBackgroundMaterial(.ultraThin)
    /// ```
    @ViewBuilder
    public func uniBackgroundMaterial(_ type: UniMaterialType = .regular) -> some View {
        self.background(UniMaterial(type))
    }

    /// Applies an uni material as the foreground style of the view.
    ///
    ...
    /// Text("Glass Text")
    ///     .font(.title)
    ///     .uniForegroundMaterial(.thick)
    /// ```
    @ViewBuilder
    public func uniForegroundMaterial(_ type: UniMaterialType = .regular) -> some View {
