...
/// An uni component that presents hierarchical data by iterating through children.
///
/// `UniOutlineGroup` provides a bridge for the `OutlineGroup` API:
...
/// Example:
/// ```swift
/// UniOutlineGroup(fileSystemItems, children: \.children) { item in
///     Label(item.name, systemImage: item.isDirectory ? "folder" : "doc")
/// }
/// ```
public struct UniOutlineGroup<
    Data: RandomAccessCollection, ID: Hashable, Parent: View, Leaf: View
>: View where Data.Element: Identifiable, Data.Element.ID == ID {
    private let data: Data
    private let children: KeyPath<Data.Element, Data?>
    private let content: (Data.Element) -> Parent
    private let isLeaf: Bool

    /// Creates an uni outline group from a collection of hierarchical data.
    ///
    /// - Parameters:
