...
struct ExploreSwiftUITests {
    ...
    @Test func testIsAvailableMajor() async throws {
        ...
        let major = UniPlatformVersion.currentMajor
        
        // checking the current major should be true on any platform we run on
        #expect(UniPlatformVersion.isAvailable(iOS: major, macOS: major, tvOS: major, watchOS: major, visionOS: major) == true)
        
        // checking a far future major should be false
        #expect(UniPlatformVersion.isAvailable(iOS: 999, macOS: 999, tvOS: 999, watchOS: 999, visionOS: 999) == false)
    }
    ...
    @Test func testIsAvailableTuple() async throws {
        ...
        #expect(UniPlatformVersion.isAvailable(iOS: (major, 0), macOS: (major, 0), tvOS: (major, 0), watchOS: (major, 0), visionOS: (major, 0)) == true)
        
        // checking a far future version should be false
        #expect(UniPlatformVersion.isAvailable(iOS: (999, 0), macOS: (999, 0), tvOS: (999, 0), watchOS: (999, 0), visionOS: (999, 0)) == false)
    }

    @Test func testRunIf() async throws {
        var executed = false
        UniPlatformVersion.runIf(true) {
            executed = true
        }
        #expect(executed == true)
        
        var executedFalse = false
        UniPlatformVersion.runIf(false) {
            executedFalse = true
        }
        #expect(executedFalse == false)
        
        let value = UniPlatformVersion.runIf(true, then: { "yes" }, else: { "no" })
        #expect(value == "yes")
        
        let value2 = UniPlatformVersion.runIf(false, then: { "yes" }, else: { "no" })
        #expect(value2 == "no")
    }
