...
/// A collection of uni modifiers for text and typography.
///
...
extension Text {
    ...
    /// Applies uni tracking (letter spacing) to the text.
    ///
    ...
    public func uniTracking(_ amount: CGFloat) -> Text {
    ...
    /// Applies uni kerning to the text.
    ///
    ...
    /// - Note: Available since iOS 14.0. Gracefully returns the original text on older systems.
    public func uniKerning(_ amount: CGFloat) -> Text {
        if #available(iOS 14.0, macOS 11.0, watchOS 7.0, tvOS 14.0, *) {
    ...
    /// Applies uni italic style to the text.
    public func uniItalic(_ active: Bool = true) -> Text {
        if active {
            return self.italic()
        } else {
            return self
    ...
    /// Applies uni bold style to the text.
    public func uniBold(_ active: Bool = true) -> Text {
        if active {
            return self.bold()
        } else {
            return self
...
extension View {
    ...
    /// Applies uni tracking (letter spacing) to a view.
    /// - Note: Tracking is primarily a Text modifier; this is a no-op fallback for generic Views.
    @ViewBuilder
    public func uniTracking(_ amount: CGFloat) -> some View {
    ...
    /// Applies uni line spacing to a view.
    ///
    ...
    @ViewBuilder
    public func uniLineSpacing(_ amount: CGFloat) -> some View {
    ...
    @ViewBuilder
    public func uniItalic(_ active: Bool = true) -> some View {
        if active {
    ...
    @ViewBuilder
    public func uniBold(_ active: Bool = true) -> some View {
        if active {
            if #available(iOS 16.0, macOS 13.0, watchOS 9.0, tvOS 16.0, *) {
                self.bold()
            } else {
    ...
    @ViewBuilder
    public func uniSemi(_ active: Bool = true) -> some View {
        if active {
            if #available(iOS 16.0, macOS 13.0, watchOS 9.0, tvOS 16.0, *) {
