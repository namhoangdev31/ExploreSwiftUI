...
/// An uni gauge component that displays a value within a range.
///
/// `UniGauge` provides a unified way to render gauges across platforms:
/// - **Modern OS (iOS 16+, macOS 13+, watchOS 7+)**: Leverages the native `Gauge` component
///   with support for all 8 uni styles.
...
/// Example:
/// ```swift
/// UniGauge(value: 0.75, in: 0...1, style: .accessoryCircular) {
///     Text("Battery")
/// } currentValueLabel: {
///     Text("75%")
/// }
/// ```
public struct UniGauge<
    Label: View,
...
>: View {
    ...
    private let style: UniGaugeStyle
    ...
    /// Creates an uni gauge with full control over labels and range.
    ///
    /// - Parameters:
    ///   - value: The current value to display.
    ///   - range: The valid range for the value.
    ///   - style: The uni gauge style to apply.
    ...
    /// Creates an uni gauge with only a value and label.
    public init(
        value: Double,
        style: UniGaugeStyle = .automatic,
        @ViewBuilder label: @escaping () -> Label
    ...
    /// Creates an uni gauge with a value, range, and label.
    public init(
        value: Double,
    ...
    /// Creates an uni gauge with a value, label, and current value label.
    public init(
        value: Double,
        style: UniGaugeStyle = .automatic,
        @ViewBuilder label: @escaping () -> Label,
