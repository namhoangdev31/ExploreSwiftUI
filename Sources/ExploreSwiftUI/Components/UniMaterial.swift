...
///
/// `UniMaterial` provides a unified way to apply blur and vibrancy effects:
...
/// Text("Glass Overlay")
///     .padding()
///     .background(UniMaterial(.ultraThin))
///     .cornerRadius(12)
/// ```
public struct UniMaterial: View {
    private let type: UniMaterialType

    /// Creates an uni material of the specified type.
    ///
    /// - Parameter type: The thickness/density of the material (e.g., `.thin`, `.thick`).
    public init(_ type: UniMaterialType = .regular) {
        self.type = type
    }
    ...
    #if canImport(UIKit) && !os(watchOS)
        private func blurStyle(for type: UniMaterialType) -> UIBlurEffect.Style {
    ...
    private struct MacBlurView: NSViewRepresentable {
        ...
        private func macMaterial(for type: UniMaterialType) -> NSVisualEffectView.Material {
