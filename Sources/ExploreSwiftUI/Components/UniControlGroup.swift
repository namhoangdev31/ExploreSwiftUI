...
///
/// `UniControlGroup` provides a bridge for the `ControlGroup` API:
...
///   a background plate depending on the current `uniControlGroupStyle`.
///
/// Example:
/// ```swift
...
/// }
/// ```
public struct UniControlGroup<Content: View, Label: View>: View {
    let content: () -> Content
    let label: (() -> Label)?
    ...
    @Environment(\.uniControlGroupStyle) private var styleType
...
public struct UniControlGroupTitled<Content: View>: View {
    ...
    @ViewBuilder
    public var body: some View {
        UniControlGroup(titleKey, systemImage: systemImage, content: content)
    }
}

/// A specialized control group with a custom label view.
public struct UniControlGroupLabeled<Content: View, Label: View>: View {
    private let content: () -> Content
    ...
    @ViewBuilder
    public var body: some View {
        UniControlGroup(content: content, label: label)
    }
}
