...
/// An uni menu component that presents a list of actions when triggered.
///
/// `UniMenu` provides a unified interface for menus across all Apple platforms:
...
/// Example:
/// ```swift
/// UniMenu("Options", systemImage: "ellipsis.circle") {
///     Button("Edit", systemImage: "pencil") { edit() }
///     Button("Delete", role: .destructive) { delete() }
/// }
/// ```
public struct UniMenu<Content: View, Label: View>: View {
    private let content: () -> Content
    private let label: () -> Label
    private let primaryAction: (() -> Void)?

    /// Creates an uni menu with a custom label view.
    ///
    /// - Parameters:
    ...
    /// Creates an uni menu with a localized title key.
    public init(
    ...
    /// Creates an uni menu with a localized title and a system image.
    public init(
        _ titleKey: LocalizedStringKey,
        systemImage: String,
        @ViewBuilder content: @escaping () -> Content,
