...
extension View {
    ...
    /// Sets the visual style for uni gauges.
    ///
    ...
    ///
    /// - Parameter style: The uni gauge style to apply (e.g., `.linear`, `.accessoryCircular`).
    ///
    /// Example:
    /// ```swift
    /// UniGauge(value: 0.75) {
    ///     Text("Battery")
    /// }
    /// .uniGaugeStyle(.accessoryCircular)
    /// ```
    @ViewBuilder
    public func uniGaugeStyle(_ style: UniGaugeStyle) -> some View {
        switch style {
        case .automatic:
    ...
    /// Sets the tint color or style for uni gauges.
    ///
    /// Example:
    /// ```swift
    /// MyGauge()
    ///     .uniGaugeTint(Color.orange)
    /// ```
    @ViewBuilder
    public func uniGaugeTint<S: ShapeStyle>(_ style: S) -> some View {
        #if os(iOS) || os(macOS) || os(watchOS) || os(visionOS)
            if #available(iOS 16.0, macOS 13.0, watchOS 9.0, visionOS 1.0, *) {
                self.tint(style)
            } else {
