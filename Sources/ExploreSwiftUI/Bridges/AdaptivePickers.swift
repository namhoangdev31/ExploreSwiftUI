...
extension View {
    ...
    /// ```
    @ViewBuilder
    public func adaptivePickerStyle(_ style: AdaptivePickerStyle) -> some View {
        switch style {
        case .automatic:
    ...
    @ViewBuilder
    public func adaptiveHorizontalRadioGroupLayout(isEnabled: Bool = true) -> some View {
        #if os(macOS)
            if isEnabled {
                if #available(macOS 10.15, *) {
    ...
    @ViewBuilder
    public func adaptiveDefaultWheelPickerItemHeight(_ height: CGFloat?) -> some View {
        #if os(watchOS)
            if let height {
