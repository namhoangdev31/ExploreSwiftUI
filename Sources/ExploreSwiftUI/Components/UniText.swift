...
/// A highly uni text component that provides a unified interface for data formatting.
///
/// `UniText` bridges modern SwiftUI `FormatStyle` APIs (iOS 15/16+) to older systems:
...
/// Example:
/// ```swift
/// UniText(1250.50, format: .currency(code: "USD"))
/// UniText(date: Date(), showTime: true)
/// ```
public struct UniText: View {
    ...
    /// Example:
    /// ```swift
    /// UniText(0.75, format: .percent) // Displays "75%"
    /// ```
    public init<T: BinaryFloatingPoint>(_ value: T, format: UniNumberFormat) {
    ...
    public init<T: BinaryInteger>(_ value: T, format: UniNumberFormat) {
    ...
    /// UniText(distance, width: .abbreviated) // Displays "5 km"
    /// ```
    public init<UnitType: Dimension>(
        _ measurement: Measurement<UnitType>, width: UniMeasurementWidth
    ) {
        #if os(iOS) || os(macOS) || os(watchOS) || os(tvOS) || os(visionOS)
            if #available(iOS 15.0, macOS 12.0, tvOS 15.0, watchOS 8.0, *) {
    ...
    /// Example:
    /// ```swift
    /// UniText(date: Date(), showTime: false) // Displays "May 15, 2026"
    /// ```
    public init(date: Date, showTime: Bool = false, showDate: Bool = true) {
            ...
            if #available(iOS 15.0, macOS 12.0, tvOS 15.0, watchOS 8.0, *) {
                ...
                self.content = AnyView(
                    Text(Self.formatDate(date, showTime: showTime, showDate: showDate)))
            }
        #else
            self.content = AnyView(
                Text(Self.formatDate(date, showTime: showTime, showDate: showDate)))
    ...
    /// Example:
    /// ```swift
    /// UniText(seconds: 90, pattern: .minuteSecond) // Displays "01:30"
    /// ```
    public init(seconds: TimeInterval, pattern: UniTimePattern) {
        #if os(iOS) || os(macOS) || os(watchOS) || os(tvOS) || os(visionOS)
            if #available(iOS 16.0, macOS 13.0, tvOS 16.0, watchOS 9.0, *) {
                ...
                self.content = AnyView(
                    Text(Duration.seconds(seconds), format: .time(pattern: nativePattern)))
            } else {
                self.content = AnyView(
                    Text(Self.formatDuration(seconds: seconds, pattern: pattern)))
            }
        #else
    ...
    private static func formatNumber(_ value: Double, format: UniNumberFormat) -> String {
    ...
    private static func formatMeasurement<UnitType: Dimension>(
        _ measurement: Measurement<UnitType>, width: UniMeasurementWidth
    ) -> String {
    ...
    private static func formatDuration(seconds: TimeInterval, pattern: UniTimePattern)
        -> String
    {
